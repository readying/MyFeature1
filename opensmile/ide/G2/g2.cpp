...
G2::G2(QWidget *parent, Qt::WFlags flags)
	: QMainWindow(parent, flags)
{
	ui.setupUi(this);

	// 将按钮和提取特征连起来
	connect(ui.pushButton,SIGNAL(clicked()), this, SLOT(SmileExtract()));

	// 将打开wav文件和open菜单连起来
	connect(ui.actionOpen,SIGNAL(triggered()), this, SLOT(OpenWav()));
...
void G2::OpenWav()
{
	wavPath = QFileDialog::getOpenFileName(this, tr(" 打开文件Open file..."), QString(), tr("wav-Files(*.wav)"));

	ui.textEdit->setText(wavPath);
}
...
{	
	...
	/*
	SimleExtract的调用方法：
	 -I E:\opensmile-2.0-rc1\msvcbuild\happy.wav -C E:\opensmile-2.0-rc1\msvcbuild\MFCC12_0_D_A.conf -O  E:\opensmile-2.0-rc1\msvcbuild\o.csv
	
	wav文件可以从界面选择；
	configure 文件放在msvcbuild里面；
	结果放在msvcbuild里面。
	
	*/

	QByteArray ba = wavPath.toLatin1();
	char * path = ba.data();
	myargv[1]="-I" ;
//	myargv[2]= "E:\\opensmile-2.0-rc1\\msvcbuild\\happy.wav";
	myargv[2] = path;
	myargv[3]="-C";
	myargv[4]="..\\..\\..\\msvcbuild\\mfcc.conf";
	myargv[5]="-O";
	myargv[6]="..\\..\\..\\msvcbuild\\mfcc.csv";
  ...
  try {
    ...
    // commandline parser:
    cCommandlineParser cmdline(myagrc,myargv);
