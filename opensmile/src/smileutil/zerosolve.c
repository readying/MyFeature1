...
static int
qr_companion (double *h, size_t nc, zerosolver_complex_packed_ptr zroot) {
  double t = 0.0;
...
next_iteration:
  ...
  if (e == n)
    {
      ZEROSOLVER_SET_COMPLEX_PACKED (zroot, n-1, x + t, 0); /* one real root */
      n--;
      goto next_root;
      /*continue;*/
    }
      ...
      if (q > 0)                /* two real roots */
        {
          ...
          ZEROSOLVER_SET_COMPLEX_PACKED (zroot, n-1, x - w / y, 0);
          ZEROSOLVER_SET_COMPLEX_PACKED (zroot, n-2, x + y, 0);
        }
      else
        {
          ZEROSOLVER_SET_COMPLEX_PACKED (zroot, n-1, x + p, -y);
          ZEROSOLVER_SET_COMPLEX_PACKED (zroot, n-2, x + p, y);
        }
      n -= 2;
...
zerosolver_poly_complex_workspace_alloc (size_t n)
{
  size_t nc ;
  ...
  w = (zerosolver_poly_complex_workspace *) 
    malloc (sizeof(zerosolver_poly_complex_workspace));
...
void 
zerosolver_poly_complex_workspace_free (zerosolver_poly_complex_workspace * w)
{
  if (w==NULL) return;
  free(w->matrix) ;
  free(w);
