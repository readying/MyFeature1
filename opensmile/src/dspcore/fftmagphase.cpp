...
int cFFTmagphase::setupNamesForField(int i, const char*name, long nEl)
{
  ...
  if (inverse) {
    ...
    addNameAppendFieldAuto(name, "fftcInv", nEl-2);
    newNEl += nEl-2; 
  } else {
    if (magnitude) {
      ...
      if (phase && joinMagphase) {
        ...
        if (dBpsd) {
          ...
          addNameAppendFieldAuto(name, "Magphase", nEl + 2);
        }
        newNEl += nEl + 2; 
      } else {
        if (dBpsd) {
  ...
  if (inverse) { // inverse is only possible from joint Mag & phase array fields... TODO: check this!
    ...
    if (doMag) {
      if (magnitude && !dBpsd && !normalise && !power) {
        ...
        dst[0] = ((FLOAT_DMEM)1.0/(FLOAT_DMEM)Nsrc)*fabs(src[0]); dst[0] *= dst[0];
        for(n=2; n<Nsrc; n += 2) {
        ...
        dst[0] = MAX((FLOAT_DMEM)-10.0, (dBpnorm + (FLOAT_DMEM)20.0 * log10( ((FLOAT_DMEM)1.0/(FLOAT_DMEM)Nsrc)*fabs(src[0]) )));
        for(n=2; n<Nsrc; n += 2) {
          dst[n/2] = MAX((FLOAT_DMEM)-10.0, (dBpnorm + (FLOAT_DMEM)10.0 * log10( ((FLOAT_DMEM)1.0/((FLOAT_DMEM)Nsrc*(FLOAT_DMEM)Nsrc))*(src[n]*src[n] + src[n+1]*src[n+1]) )) );
        }
        dst[Nsrc/2] = MAX((FLOAT_DMEM)-10.0, (dBpnorm + (FLOAT_DMEM)20.0 * log10( ((FLOAT_DMEM)1.0/(FLOAT_DMEM)Nsrc)*fabs(src[1]) )));
        //dst += Nsrc/2+1;
      }
    }
    if (phase) {
