...
void cLibsvmLiveSink::processResult(lsvmDataFrame *f, int modelIdx, int mmm /* multiModelMode */)
{
  ...
  int clIdx = -1;
  if (printResult || sendResult || saveResult) {
  ...
  if (printResult||saveResult) {
    ...
    if ((models[modelIdx].nClasses > 0)&&(models[modelIdx].classNames != NULL)) {
      ...
      if (saveResult) {
        fprintf(fl," LibSVM  '%s' (#%i = %s) result [final=%i] (@ time: %f) :  ~~> %s <~~",getInstName(),modelIdx,models[modelIdx].modelResultName,f->isFinal,f->time,models[modelIdx].classNames[clIdx]);
      }
    } else {
      if (printResult) {
      ...
      if (saveResult) {
        fprintf(fl," LibSVM  '%s' (#%i = %s) result [final=%i] (@ time: %f) :  ~~> %.2f <~~",getInstName(),modelIdx,models[modelIdx].modelResultName,f->isFinal,f->time,f->res);
      }
    }
    if (f->probEstim != NULL) {
      int i;
