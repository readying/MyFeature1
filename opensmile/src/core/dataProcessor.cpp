...
// (optional) fullName = 1: match full field name instead of part of name
long cDataProcessor::findField(const char *namePartial, int fullName, long *N, const char **fieldName, long nEl, int *more)
{
  const FrameMetaInfo * fmeta = reader_->getFrameMetaInfo();
  int ri=0;
  long idx;
...
//
long cDataProcessor::findElement(const char *namePartial, int fullName, long *N, const char **fieldName, int *more)
{
  const FrameMetaInfo * fmeta = reader_->getFrameMetaInfo();
  int ri=0;
  long idx;
