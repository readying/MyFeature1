  ...
  if (multiOut==1) {
    ...
    if (isMessageType(_msg,"turnStart")) {
      ...
      if (showSegmentTimes_) {
        double start_t = _msg->floatData[2] * (double)vIdxStart;
        printf("Start of segment: %.2f seconds\n", start_t);
      }
      return 1;
    }
    if (isMessageType(_msg,"turnEnd")) {
      if (!turnEnd) { // if previous message was not yet processed we discard this message
        ...
        turnEnd = 1;
        if (showSegmentTimes_) {
          double start_t = _msg->floatData[2] * (double)vIdxEnd;
          printf("End of segment: %.2f seconds\n", start_t);
        }
