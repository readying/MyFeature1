...
SMILECOMPONENT_REGCOMP(cFunctionalPeaks2)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ct->setField("numPeaks","1/0=enable/disable output of number of peaks",0);
    ...
    ct->setField("peakMeanMeanDist","1/0=enable/disable output of arithmetic mean of peaks - arithmetic mean of all values",0);
    ct->setField("peakMeanRel","1/0=enable/disable output of arithmetic mean of peaks (local maxima) / arithmetic mean of all values",0);
    ct->setField("ptpAmpMeanAbs","1/0=enable/disable output of mean peak to peak (amplitude) difference",0);
    ct->setField("ptpAmpMeanRel","1/0=enable/disable output of mean peak to peak (amplitude) difference / arithmetic mean",0);
    ct->setField("ptpAmpStddevAbs","1/0=enable/disable output of mean peak to peak (amplitude) standard deviation",0);
    ct->setField("ptpAmpStddevRel","1/0=enable/disable output of mean peak to peak (amplitude) standard deviation / arithmetic mean",0);
    ...
    ct->setField("mtmAmpMeanRel","1/0=enable/disable output of mean minimum to minimum (amplitude) difference / arithmetic mean",0);
    ct->setField("mtmAmpStddevAbs","1/0=enable/disable output of mean minimum to minimum (amplitude) standard deviation",0);
    ct->setField("mtmAmpStddevRel","1/0=enable/disable output of mean minimum to minimum (amplitude) standard deviation",0);
...
void cFunctionalPeaks2::dbgPrintMinMaxList(struct peakMinMaxListEl * listEl)
{
  if (dbg != NULL) {
    ...
    while (listEl != NULL) {
      if (listEl->type == 1)
        fprintf(dbg,"XXXX_MAX: x=%i y=%f\n",listEl->x,listEl->y);
      else
        fprintf(dbg,"XXXX_MIN: x=%i y=%f\n",listEl->x,listEl->y);
      listEl = listEl->next;
    }
  } else if (consoleDbg) {
    printf("---\n");
