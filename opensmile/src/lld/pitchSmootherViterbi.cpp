...
SMILECOMPONENT_REGCOMP(cPitchSmootherViterbi)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
   ...
   ct->setField("F0finalLog","1 = Enable output of final (corrected and smoothed) F0 (and also F0finalEnv - both only if enabled) in log representation (semitone scale with base note 55.0 Hz) instead of linear scaling.", 0);
   ct->setField("F0finalEnv","1 = Enable output of envelope of final smoothed F0 (i.e. there will be no 0 values (except for end and beginning)). [NOT IMPLEMENTED!]",0);
  ...
  if (avail > 0) {
    ...
    for (i=0; i<avail; i++) {
      ...
      if (F0final) {
        ...
        if (F0finalLog) {
          if (f0 > 0.0) {
            // semi-tone scale, base tone 50 Hz
            f0scaled = 12.0 * log(f0 / 55.0) / log(2.0);
          }
